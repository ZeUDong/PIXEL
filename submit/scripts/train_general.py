...
def get_output_and_loss_general(method, model, criterion,
                                ...
                                text_input_ids=None, text_attention_mask=None, text_mask_labels=None, text_replace_labels=None,
...
def train_hashing(optimizer, model, train_loader, device, loss_name, loss_cfg, onehot,
    ...
    for i, batch in enumerate(pbar):
        ...
        data, labels, index, img_paths, text_input_ids, text_attention_mask, text_mask_labels, text_replace_labels = batch[:8]
        # print(img_paths)
        # bb
        data, labels = data.to(device), labels.to(device)
        text_input_ids, text_attention_mask, text_mask_labels, text_replace_labels = text_input_ids.to(device), text_attention_mask.to(device), text_mask_labels.to(device), text_replace_labels.to(device)
    ...
    for it in range(niters):
        ...
        for ep in range(config['start_epoch_from'], nepochs):
            ...
            if eval_now:
                ...
                data = test_out['codes'].cpu().numpy()
                label = test_out['labels'].cpu().numpy()
                label = np.argmax(label, axis=-1)
                #print(data.shape) 1000,64
                result = tsne.fit_transform(data)
                #print(result.shape) 1000,2

                # label10 = label[label<10]
                # result10 = result[label<10]
                #digits = load_digits()
                #print(np.unique(label))
                plt.figure(dpi=120,figsize=(5,5))
                plt.scatter(result[:,0], result[:,1], c=label,label="t-SNE")
                plt.tight_layout()
